...
package com.mobswing.nes.model
{
	...
	public class PPU
	{
		...
		private var bgbuffer		:Vector.<uint> = new Vector.<uint>(Globals.WIDTH*Globals.HEIGHT);
		private var pixrendered		:Vector.<int> = new Vector.<int>(Globals.WIDTH*Globals.HEIGHT);
		private var spr0dummybuffer	:Vector.<int> = new Vector.<int>(Globals.WIDTH*Globals.HEIGHT);
		private var dummyPixPriTable:Vector.<int> = new Vector.<int>(Globals.WIDTH*Globals.HEIGHT);
		private var oldFrame		:Vector.<int> = new Vector.<int>(Globals.WIDTH*Globals.HEIGHT);
		public	var buffer			:Vector.<uint>;
		private var tpix			:Vector.<int>;

		public	var scanlineChanged	:Vector.<Boolean> = new Vector.<Boolean>(Globals.HEIGHT);
		...
		public	function startVBlank():void
		{
			...
			{
				renderFramePartially(this.buffer, this.lastRenderedScanline+1, Globals.HEIGHT-this.lastRenderedScanline);
			}
		...
		public	function endScanline():void
		{
			...
			else if ((scanline >= (21+vblankAdd)) && (scanline <= 260))
			{
 				if (this.f_bgVisibility == 1)
				{
					...
					{
						if ((this.sprX[0]>=-7) && (this.sprX[0]<Globals.WIDTH) && (this.sprY[0]+1<=(this.scanline-this.vblankAdd+1-21)) && ((this.sprY[0]+1+(this.f_spriteSize==0 ? 8:16)) >= (this.scanline-this.vblankAdd+1-21)))
						{
		...
		public	function endFrame():void
		{
			...
			if (this.showSpr0Hit)
			{
				if ((this.sprX[0]>=0) && (this.sprX[0]<Globals.WIDTH) && (this.sprY[0]>=0) && (this.sprY[0]<Globals.HEIGHT))
				{
				...
				if ((this.spr0HitX>=0) && (this.spr0HitX<Globals.WIDTH) && (this.spr0HitY>=0) && (this.spr0HitY<240))
				{
					for(i = 0 ; i < Globals.WIDTH ; i++)
					{
		...
		private	function renderFramePartially(buffer:Vector.<uint>, startScan:int, scanCount:int):void
		{
			...
			if (!this.requestRenderAll)
			{
				...
				if (startScan+scanCount > Globals.HEIGHT)
					scanCount = Globals.HEIGHT - startScan;
				for (i = startScan ; i <  startScan+scanCount ; i++)
				{
					this.scanlineChanged[i] = false;
					this.si = i<<8;
					jmax = this.si+Globals.WIDTH;
		...
		private	function renderBgScanline(buffer:Vector.<uint>, scan:int):void
		{
			...
			if (scan<Globals.HEIGHT && (scan-this.cntFV)>=0)
			{
		...
		private	function renderSpritesPartially(startscan:int, scancount:int, bgPri:Boolean):void
		{
			if (f_spVisibility == 1)
			{
				...
				{
					if ((bgPriority[i]==bgPri) && (sprX[i]>=0) && (sprX[i]<Globals.WIDTH) && (sprY[i]+8>=startscan) && (sprY[i]<startscan+scancount))
					{
						if (f_spriteSize == 0)
						{
							srcy1 = 0;
